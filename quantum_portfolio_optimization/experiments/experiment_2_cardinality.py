...
def run_cardinality_experiment(n_assets: int = 10, 
                              max_assets: int = 5,
                              target_return: Optional[float] = None,
                              n_bits: int = 3,
                              n_repetitions: int = 3) -> Dict:
    """
    Run portfolio optimization experiment with cardinality constraints.
    ...
    """
    print(f"Running Portfolio Optimization Experiment with Cardinality Constraints")
    ...
    # Run classical optimization with cardinality constraint
    print("\nRunning classical optimization with cardinality constraint...")
    classical_results = []
    ...
    print("\nRunning quantum optimization with cardinality constraint...")
    quantum_results = []
    ...
    for i in range(n_repetitions):
        ...
        qubo_matrix = qubo.build_qubo_matrix(encoding_matrix, cardinality_constraint=max_assets)
        ...
        print(f"  - Run {i+1}/{n_repetitions}: Risk={quantum_result['portfolio_risk']:.6f}, Num Assets={quantum_result['num_assets']}, Time={quantum_result['run_time']:.6f}s")
    ...
    avg_quantum = {
        ...
        'num_assets': np.mean([r['num_assets'] for r in quantum_results]),
    ...
    fig1 = PortfolioVisualization.plot_portfolio_weights(
        avg_classical['weights'],
        title=f"Classical Portfolio Weights (Cardinality = {max_assets})"
    )
    
    fig2 = PortfolioVisualization.plot_portfolio_weights(
        avg_quantum['weights'],
        title=f"Quantum Portfolio Weights (Cardinality = {max_assets})"
    )
...
if __name__ == "__main__":
    ...
    # Save figures
    results['figures']['classical_weights'].savefig('classical_weights_cardinality.png')
    results['figures']['quantum_weights'].savefig('quantum_weights_cardinality.png')
